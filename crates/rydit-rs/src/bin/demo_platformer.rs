...
fn main() {
    ...
    while running {
            ...
            match event {
                ...
                sdl2::event::Event::KeyDown { keycode: Some(key), repeat: false, .. } => {
                    match key {
                        ...
                        sdl2::keyboard::Keycode::W | sdl2::keyboard::Keycode::Up | sdl2::keyboard::Keycode::Space => {
                            if en_suelo {
                                velocidad_y = salto;
                                en_suelo = false;
                            }
                ...
                sdl2::event::Event::KeyDown { keycode: Some(key), repeat: true, .. } => {
                    match key {
                        sdl2::keyboard::Keycode::D | sdl2::keyboard::Keycode::Right => {
                            velocidad_x = 200.0;
                        }
        ...
        if jugador.x > 750.0 { jugador.x = 750.0; }
        ...
        canvas.fill_rect(sdl2::rect::Rect::new(0, 550, 800, 50)).ok();

        // Dibujar plataformas (decoración)
        canvas.set_draw_color(sdl2::pixels::Color::RGB(80, 80, 80));
        canvas.fill_rect(sdl2::rect::Rect::new(200, 450, 150, 20)).ok();
        canvas.fill_rect(sdl2::rect::Rect::new(450, 380, 150, 20)).ok();
        canvas.fill_rect(sdl2::rect::Rect::new(100, 300, 150, 20)).ok();
