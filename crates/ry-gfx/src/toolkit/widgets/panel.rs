...
impl Panel {
    ...
    pub fn render(&self, backend: &mut Sdl2Backend, theme: &Theme) {
        ...
        if let Some(ref title) = self.title {
            // Barra de título
            let title_height = 24;
            backend.set_draw_color(theme.title_bar_bg);
            let title_rect = Rect::new(self.x, self.y, self.width as u32, title_height as u32);
            let _ = backend.fill_rect(title_rect);
            ...
            backend.set_draw_color(theme.title_text);
