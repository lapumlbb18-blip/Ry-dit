...
pub fn ejecutar_programa_gfx<'a>(
    ...
    ry_gfx::debug_log::debug_init(
        "/data/data/com.termux/files/home/shield-project/ry_debug.log",
    );
    ry_gfx::debug_log::debug_info("=== INICIANDO GAME LOOP ===");
    ...
    ry_gfx::debug_log::debug_log(&format!(
    ...
    for stmt in &program.statements {
        match stmt {
            ...
            Stmt::While { .. } | Stmt::Block(_) => {
                ry_gfx::debug_log::debug_info("Statement es game loop (While/Block)");
            }
            // Todo lo demás es inicialización
            _ => {
                ry_gfx::debug_log::debug_log("Ejecutando statement inicial");
    ...
    for stmt in &program.statements {
        match stmt {
            Stmt::While { condition, body } => {
                found_loop = true;
                ry_gfx::debug_log::debug_info("=== ENCONTRADO GAME LOOP WHILE ===");
                ry_gfx::debug_log::debug_log(&format!("Body tiene {} statements", body.len()));
                ...
                loop {
                    ...
                    let escape = gfx.is_key_pressed(ry_gfx::Key::Escape);
                    ...
                    for s in body {
                        ...
                        if break_signal == Some(true) {
                            ry_gfx::debug_log::debug_log("Break detectado en body");
                            break;
                        }
                    }
                    ...
                    // Log cada 10 frames
                    if frame_count % 10 == 0 {
                        ry_gfx::debug_log::debug_log_frame(&format!(
