        ...
        while i < chars.len() {
            ...
            if chars[i].is_alphabetic() || chars[i].is_alphanumeric() || chars[i] == '_' {
                ...
                let token = match ident.as_str() {
                    ...
                    "return" => Token::Return,
                    "voz" => Token::Voz,
                    // Draw commands (con aliases en español)
                    "draw.circle" | "dibujar.circulo" => Token::DrawCircle,
                    "draw.rect" | "dibujar.rect" | "dibujar.rectangulo" => Token::DrawRect,
                    "draw.line" | "dibujar.linea" => Token::DrawLine,
...
impl Parser {
    ...
    fn parse_statement(&mut self) -> Result<Option<Stmt>> {
        ...
        match &self.tokens[self.pos] {
            // Saltar comentarios - no son statements
            Token::Comentario(_) => {
                self.pos += 1;
                return Ok(None);
            }
    ...
    fn parse_block(&mut self) -> Result<Option<Stmt>> {
        ...
        while self.pos < self.tokens.len() {
            ...
            if let Some(stmt) = self.parse_statement()? {
                ...
                // Token no es un statement válido - NO avanzar, solo break
                // Avanzar aquí consume tokens que pertenecen al padre
                break;
            }
        }
